...
def build_target(args):
    ...
    center_z : 도형 중심 높이 (테이블 위에 세웠을 때)
    grasp_z  : TCP를 보낼 높이 (몸통 중앙 감싸쥠)
    grasp_width : 파지 방향 폭 → 닫힘값 계산에 사용
    """
    if args.shape == "cylinder":
        h, r = args.height, args.radius
        center_z = TABLE_TOP_Z + h / 2
        return "cylinder", (h, r), center_z, center_z, 2 * r
    ...
    # 그리퍼가 x축 방향으로 벌어지므로 파지 폭은 sx
    return "box", (sx, sy, sz), center_z, center_z, sx
