...
| 시뮬레이터 | Gazebo Classic 11 + MoveIt2 demo (RViz) |

> **참고 (작성 배경)** — 이 가이드는 Ubuntu 24.04 + ROS2 Jazzy 환경에서 검증한 과정을
> Humble 환경에 맞게 옮긴 것입니다. 주요 차이:
> 저장소 브랜치(`jazzy` → `humble`), Gazebo(새 Gazebo `ros-gz` 계열 → **Gazebo Classic** `gazebo-ros` 계열),
> apt 패키지 접두어(`ros-jazzy-*` → `ros-humble-*`), pip 옵션(`--break-system-packages` 불필요).
