...
| 키 | 저장소 기본값 | **바꿀 값** | 의미 |
|---|---|---|---|
| `arm_ip` | `"192.168.1.17"` | `"192.168.1.18"` | 로봇 컨트롤러의 IP — 드라이버가 명령을 보낼 주소 |
| `udp_ip` | `"192.168.1.10"` | `"192.168.1.100"` | **PC의 유선 IP** — 로봇이 상태(관절값 등)를 UDP로 밀어 넣을 주소 |

```bash
cd ~/ros2_ws/src/ros2_rm_robot/rm_driver/config
sed -i 's/arm_ip: "192.168.1.17"/arm_ip: "192.168.1.18"/; s/udp_ip: "192.168.1.10"/udp_ip: "192.168.1.100"/' rm_75_config.yaml
...
**① 상태 보고 주기가 200 Hz다** (시뮬은 100 Hz)

`ros2 topic hz /joint_states`가 **200**을 가리킨다. 주기를 정하는 주체가 다르기 때문이다:
