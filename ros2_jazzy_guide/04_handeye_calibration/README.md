# 04. 비전 준비 — 뎁스카메라 테스트부터 Eye-in-Hand 캘리브레이션까지
...
PyTorch는 **GPU(CUDA) 빌드**로 설치한다 (RTX 5080 기준 cu128):
...
스크립트는 저장소의 `04_handeye_calibration/scripts/`에 있다. `~/robot_vision/`에 복사해 사용한다.
...
requirements의 pyrealsense2 버전이 구형이라 충돌한다. 최신으로 바꾼 뒤 설치:
...
우리 실험실에서 실제로 밟은 과정이다. **오차가 15 mm를 넘으면 이 표에서 자기 상황을 찾아라.**
